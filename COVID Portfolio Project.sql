--Queries for Tableau/Collecting Useful data fror visualization 
--1. Total cases and total death percentage
...
--2. Each continents total death
Select location, SUM(CAST(new_deaths as int)) as total_deaths
From PortfolioProject.dbo.CovidDeaths
Where continent is null and location not in('World', 'European Union', 'International','Upper middle income', 'High income','Lower middle income',
'Low income')
Group by location 
order by total_deaths desc


--3. Highest percentage of population infected per country ranking 
Select Location,Population,SUM(new_cases) as HighestInfectionCount, 
(SUM(new_cases) / Population *100) as PercentageInfectedPerPop
From PortfolioProject.dbo.CovidDeaths
Where continent is not null
Group by Location, Population 
Order by PercentageInfectedPerPop DESC



--4. Timeline per day where covid cases increase
